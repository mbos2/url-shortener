...
import { createShortUrlRecord, deleteShortUrlRecord, getUrlsByAlias } from './common/post-utils.js';
export default async ({ req, res, log, error }) => {
    ...
    if (req.method === 'POST' && req.path === '/search') {
        ...
        if (!payload.alias) {
            ...
            return res.json({ ok: false, message: `No alias was found in the payload.` }, 400);
        }
        const alias = payload.alias;
